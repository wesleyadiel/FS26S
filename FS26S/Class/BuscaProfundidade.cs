...
namespace FS26S.Class
{
    public class BuscaProfundidade
    {
        ...
        public void IniciarBuscar(City origem, City destino, ref decimal distanciaTotal, ref decimal conexaoMaisCurta, ref string caminho)
        {
            ...
            Buscar(origem, destino, ref distanciaTotal, ref conexaoMaisCurta, ref caminho);

            caminho = $"{origem.name}" + caminho;
        }

        private bool Buscar(City city, City destino, ref decimal distancia, ref decimal conexaoMaisCurta, ref string caminho)
        {
            CitiesVisited.Add(city.code);

            foreach (City cityFor in city.conections)
            {
                cityFor.conections = Cities.Where(c => c.code == cityFor.code).FirstOrDefault()?.conections;
            }

            if (city.conections.Where(c => c.code == destino.code).Count() == 1)
            {
                if (conexaoMaisCurta == 0.00M || conexaoMaisCurta > city.conections.Where(c => c.code == destino.code).FirstOrDefault().distance)
                {
                    conexaoMaisCurta = city.conections.Where(c => c.code == destino.code).FirstOrDefault().distance;
                }

                distancia += city.conections.Where(c => c.code == destino.code).FirstOrDefault().distance;
                caminho = $" -> {city.conections.Where(c => c.code == destino.code).FirstOrDefault().name}" + caminho;
                return true;
            }

            foreach (City cityFor in city.conections.Where(c => c.code != city.code && !CitiesVisited.Contains(c.code)))
            {
                decimal distanciaFor = 0.00M;
                if (Buscar(cityFor, destino, ref distanciaFor, ref conexaoMaisCurta, ref caminho))
                {
                    if (conexaoMaisCurta == 0.00M || conexaoMaisCurta > cityFor.distance)
                    {
                        conexaoMaisCurta = cityFor.distance;
                    }

                    caminho = $" -> {cityFor.name}" + caminho;
                    distancia += distanciaFor + cityFor.distance;
                    return true;
                }
            }
